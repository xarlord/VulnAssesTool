...
// Multiple parameters
When(
  'I query CVEs with severity filter {string} and CVSS range {float} to {float}',
...
Then(/results should be ordered by (.*) (ascending|descending)/, function (field: string, order: string) {
...
// NOT
Given('the database is initialized', async () => {
  // ✗ arrow function
  this.db = new NvdDatabase()
})
```
