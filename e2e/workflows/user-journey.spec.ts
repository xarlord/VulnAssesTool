...
test.describe('Full User Journey', () => {
  test('complete vulnerability assessment journey', async ({ page }) => {
    ...
    await expect(page.getByText('lodash')).toBeVisible({ timeout: 10000 })
    await expect(page.getByText('axios')).toBeVisible({ timeout: 5000 })
    await expect(page.getByText('express')).toBeVisible({ timeout: 5000 })
  ...
  test('multi-project data isolation', async ({ page }) => {
    ...
    await navigateToComponentsTab(page)
    await page.waitForTimeout(E2E_UI_DELAY)
    await expect(page.getByText('axios')).toBeVisible({ timeout: 10000 })
    ...
    // Delete project 2
    await deleteProject(page, project2)
